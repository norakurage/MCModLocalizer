...
- 改行や \\n は原文通り保持
- 返答は必ず JSON（{\"translations\": [...]} の形）で返す
"""

USER_TEMPLATE = """以下の items は { \"key\":..., \"value\":... } の配列です。
出力は **単一の JSON オブジェクトのみ** とし、構造は次の通りです。
- プロパティは必ず \"translations\" のみ
- translations は item の並び順に対応した日本語訳（保護トークン ‹Tn› は原文どおり保持）を格納した配列
【入力例】
items:
[
...
def translate_batch(
            ...
            "schema": {
                ...
                "properties": {
                    "translations": {
                        "type": "array",
                        "items": {"type": "string"},
                        "minItems": len(unique_keys),
                        "maxItems": len(unique_keys),
                    }
                },
        ...
        try:
            ...
            if isinstance(obj, dict):
                if "translations" in obj and isinstance(obj["translations"], list):
                    arr = [str(v) for v in obj["translations"]]
                    return {
                        str(unique_keys[i]): arr[i]
                        for i in range(min(len(unique_keys), len(arr)))
                    }
        ...
        try:
            ...
            if with_response_format:
                return _call_responses(
                    False,
                    extra_note + "\n出力は必ず『単一の JSON オブジェクト（translations 配列のみを含む）』で返してください。"
                )
            raise
    ...
    inter = sum(1 for k in unique_keys if k in data and data[k])
    if inter < len(unique_keys):
        note = ("\n出力は次の形式のみ：{\"translations\": [<item.value の日本語訳>...]}。余計な文字や説明は一切書かないこと。")
